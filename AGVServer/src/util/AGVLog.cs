...
namespace AGV.util {
	public class AGVLog {
		...
		private static void WriteLogs(string type,string content) {
			lock (obj) {
				string filepath = getOrCreateFile(AppDomain.CurrentDomain.BaseDirectory + "log" + "\\" + DateTime.Now.ToString("yyMM"),DateTime.Now.ToString("dd") + ".txt");
		...
		/// <param name="type">日志类型</param>
		/// <param name="content"></param>
		public static void WriteSendLogs(string type,string content) {
			lock (obj) {
				string filepath = getOrCreateFile(AppDomain.CurrentDomain.BaseDirectory + "log" + "\\" + "send" + "\\" + DateTime.Now.ToString("yyMM"),DateTime.Now.ToString("dd") + ".txt");
				if (File.Exists(filepath)) {
					StreamWriter sw = new StreamWriter(filepath,true,System.Text.Encoding.Default);
					Console.WriteLine("content: " + content);
					sw.WriteLine(DateTime.Now + " " + type);
